...
Shipped work: Loopa (loopa.nutriyah.com), a sparkling drinks brand site with a distributor bulk-order flow; Nutriyah (nutriyah.com), the parent food and beverage company site; and Sri Cauvery Electronics (sricauveryelectronics.shop), a bilingual Tamil/English e-commerce store with cart, cash-on-delivery checkout, PWA install and an admin app, built on Next.js, Prisma and PostgreSQL. There is also Ember & Oak (cafecaffeine.vercel.app), a demo concept site for a specialty coffee house built with React, Vite, Tailwind and Framer Motion and Dental Health (cleardental.vercel.app), a demo dental clinic site with an appointment form backed by Express and Nodemailer — both are portfolio demonstrations, not client projects. Élaré Beauty (elarebeauty.store) is a makeup e-commerce build currently in progress — storefront, admin back-office and API, with Razorpay and cash-on-delivery checkout.
